...
def expand_fringe(current_state, fringe):
    for move in [(0, 1), (0, -1), (-1, 0), (1, 0)]:
        new_board = current_state.board.duplicate()
        new_board = new_board.slide_blank(move)
        if new_board is not None:
            new_state = State.State(new_board,
                                    current_state,
...
def breadth_first_search(fringe, max_depth, goal_board):
    ...
    if current_state.depth > max_depth:
        return CONTINUE
    if current_state.board is goal_board:
...
def a_star_f_function_factory(heuristic, goal_board):
    return lambda board, current_depth: (current_depth + heuristic(board, goal_board))
    ...
    for goal_r in range(len(goal_board.matrix)):
        for goal_c in range(len(goal_board.matrix[0])):
            ...
            if val == 0:
                continue
            current_r, current_c = current_board.find_element(val)
            total += (abs(goal_r - current_r) + abs(goal_c - current_c)) ** 2
...
def informed_expansion(current_state, fringe, f_function):
    for move in [(0, 1), (0, -1), (-1, 0), (1, 0)]:
        new_board = current_state.board.duplicate().slide_blank(move)
        if new_board is not None:
            new_state = State.State(new_board,
                                    current_state,
                                    (current_state.depth + 1),
                                    f_function(new_board,
                                               (current_state.depth + 1)))
...
def informed_search(fringe, goal_board, f_function, explored):
    ...
    if explored.get(current_state.board) is not None:
        if current_state.fvalue < explored.get(current_state.board):
...
def ids(start_board, goal_board, final_depth):
    for k in range(final_depth):
        fringe = [State.State(start_board, None, 0, 0)]
        found = CONTINUE
        while found == CONTINUE:
            found = depth_first_search(fringe, k, goal_board)
        if isinstance(found, State.State):
            # Found goal!
            return found
        # Max depth reached...
...
# Returns STOP if the fringe is empty
# Expands the first node in the fringe not past the max depth
def depth_first_search(fringe, max_depth, goal_board):
    if not fringe:
        return STOP
    for state in fringe:
        if state.depth < max_depth:
            if state.board is goal_board:
                return state
            expand_fringe_depth(state, fringe)
            fringe.remove(state)
            return CONTINUE


# Expands the fringe according to the DFS algorithm needs, adding new nodes to the beginning of the fringe
def expand_fringe_depth(current_state, fringe):
    for move in [(0, 1), (0, -1), (-1, 0), (1, 0)]:
        new_board = current_state.board.duplicate()
        new_board = new_board.slide_blank(move)
        if new_board is not None:
            new_state = State.State(new_board, current_state, (current_state.depth + 1), 0)
...
def main():
    ...
    print("testing informed expansion")
    node1 = State.State(simple_board, None, 0, 0)
    fringe1 = []
