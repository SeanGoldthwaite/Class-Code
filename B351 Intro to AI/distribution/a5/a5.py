...
def calc_entropy(classifications):
    entropy = 0
    freqs = unique(classifications)[1]
    total = sum(freqs)
    for item in freqs:
        entropy -= (item / total) * math.log2(item / total)
    return entropy
...
def calc_information_gain(parent_classifications, classifications_by_val, val_freqs):
    ...
    for val in classifications_by_val.values():
        parent_entropy -= (calc_entropy(val) * (len(val) / len(parent_classifications)))
...
class Node:
    ...
    def classify_point(self, point):
        ...
        attr = point[self.attribute]
        for child in self.children:
            if child is attr:
                return self.children[child].classify_point(point)
...
class KNN_Classifier:
    ...
    def calc_euclidean_distance(self, point_a, point_b):
        total = 0
        for item in zip(point_a, point_b):
    ...
    def get_top_label(self, top_k_labels):
        labs = unique(top_k_labels)
        maxIndex = 0
        for i in range(len(labs[0])):
            if labs[1][maxIndex] < labs[1][i]:
                maxIndex = i
        return labs[0][maxIndex]
    ...
    def classify_point(self, point, training_points, training_labels):
        ...
        for i in range(len(training_points)):
            lst.append((self.calc_euclidean_distance(point, training_points[i]), training_labels[i]))
        lst.sort(key=lambda x: x[0], reverse=True)
        lab_list = []
        for i in range(self.k):
            lab_list.append(lst.pop()[1])
        temp = self.get_top_label(lab_list)
        return temp
